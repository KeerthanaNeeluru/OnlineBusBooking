...
namespace BusBookingProject
{
    public partial class BookingReport : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            bool PnrNumberIsFound = SearchForPnrInBookingMaster(PnrNumber);
        ...
        private bool SearchForPnrInBookingMaster(string PnrNumber)
        {
            ...
            string query = "SELECT * FROM [dbo].[BookingMaster] WHERE [PNRNo] = @PnrNumber";
        ...
        {
            if (SearchForPnrInBookingMaster(PnrNumber))
            {
                if (connString.State == ConnectionState.Closed)
                {
                    connString.Open();
                }
                string deletePnrDetailsQuery = "DELETE FROM [dbo].[PNRDetails] WHERE [PNRNo] = @PnrNumber";
                string deleteBookingMasterQuery = "DELETE FROM [dbo].[BookingMaster] WHERE [PNRNo] = @PnrNumber";

                using (SqlCommand command = new SqlCommand(deletePnrDetailsQuery, connString))
                {
                    command.Parameters.AddWithValue("@PnrNumber", PnrNumber);
                    command.ExecuteNonQuery();
                }

                using (SqlCommand command = new SqlCommand(deleteBookingMasterQuery, connString))
                {
                    command.Parameters.AddWithValue("@PnrNumber", PnrNumber);
                    command.ExecuteNonQuery();
                }
