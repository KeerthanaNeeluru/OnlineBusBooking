...
CREATE PROCEDURE [dbo].[uspUpdateDataByContact]
(
    @Contact nvarchar(50),
    @PaxName nvarchar(255),
    @Email nvarchar(255),
    @SeatNo nvarchar(10),
    @TravelDate datetime,
    @Origin nvarchar(255),
    ...
    @Fare decimal,
    @BookedBy nvarchar(255),
    @BusName nvarchar(255)
)
AS
BEGIN
    ...
    SET
        Fname = CASE
            WHEN CHARINDEX(' ', @PaxName) > 0 THEN LEFT(@PaxName, CHARINDEX(' ', @PaxName) - 1)
            ELSE @PaxName
        END,
        Lname = CASE
            WHEN CHARINDEX(' ', @PaxName) > 0 THEN SUBSTRING(@PaxName, CHARINDEX(' ', @PaxName) + 1, LEN(@PaxName))
            ELSE ''
        END,
        Email = @Email,
        SeatNo = @SeatNo,
    ...
    -- Update BusMaster
    UPDATE BM
    SET BM.BusName = @BusName
    FROM dbo.BusMaster BM
    JOIN dbo.BookingMaster BK ON BM.BusId = BK.BusId
    WHERE BK.Contact = @Contact;

    -- Update Registration
    UPDATE RM
    SET
        Fname = CASE
            WHEN CHARINDEX(' ', @BookedBy) > 0 THEN LEFT(@BookedBy, CHARINDEX(' ', @BookedBy) - 1)
            ELSE @BookedBy
        END,
        Lname = CASE
            WHEN CHARINDEX(' ', @BookedBy) > 0 THEN SUBSTRING(@BookedBy, CHARINDEX(' ', @BookedBy) + 1, LEN(@BookedBy))
            ELSE ''
        END
    FROM dbo.Registration RM
    JOIN dbo.BookingMaster BK ON RM.regId = BK.RegId
    WHERE BK.Contact = @Contact;
...
CREATE PROCEDURE [dbo].[ispDeleteBookingByContact]
    @Contact VARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;

    -- Delete the booking based on the contact number
    DELETE FROM dbo.BookingMaster
    WHERE Contact = @Contact;
